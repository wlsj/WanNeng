...
import com.wls.wnlibrary.utils.kline.entry.SizeColor;
...
public class BOLLDrawing implements IDrawing{
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        if (currentIndex < maxIndex - 1) {
            ...
            r1Buffer[i * 4 + 2] = 0;
            r1Buffer[i * 4 + 3] = entrySet.getEntryList().get(currentIndex + 1).getMb();

            r2Buffer[i * 4 + 0] = 0;
            r2Buffer[i * 4 + 1] = entry.getUp();
            r2Buffer[i * 4 + 2] = 0;
            r2Buffer[i * 4 + 3] = entrySet.getEntryList().get(currentIndex + 1).getUp();

            r3Buffer[i * 4 + 0] = 0;
            r3Buffer[i * 4 + 1] = entry.getDn();
            r3Buffer[i * 4 + 2] = 0;
            r3Buffer[i * 4 + 3] = entrySet.getEntryList().get(currentIndex + 1).getDn();
        }
    }
